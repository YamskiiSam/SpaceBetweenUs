...
namespace SpaceBetweenUs.Services
{
    public class HumanDetector
    {
        ...
        public static void InitializeDetector()
        {
            if (detector == null)
            {
                AlturosYoloV3Detector alturosDetector = null;
                try
                {
                    alturosDetector = new AlturosYoloV3Detector();
                }
                catch { }
                if (alturosDetector?.DetectionSystem != Alturos.Yolo.DetectionSystem.GPU) detector = new YoloV3Detector();
                else detector = alturosDetector;
            }
        }
